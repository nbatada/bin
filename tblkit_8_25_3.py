...
def _render_vertical_header_view(df: pd.DataFrame, is_header_present: bool):
    ...
    if df.empty:
        ...
        # Stringify all cells for consistent display, handling potential NA
    ...
    for i, (h, cell) in enumerate(zip(header, first_row)):
        # Simple truncation for the cell value
...
def _handle_view(df: pd.DataFrame, args: argparse.Namespace, is_header_present: bool, **kwargs) -> Optional[pd.DataFrame]:
    """
    Handles all 'view' command logic including on-the-fly renaming, cleaning, row/column sorting, and ranking.
    """
    df_to_view = df.copy()
    current_cols = df_to_view.columns.tolist()

    # 1. On-the-fly renaming
    if getattr(args, "rename_map", None):
        pairs = re.split(r'[,;]', args.rename_map)
        rename_dict = {}
        for p in pairs:
            if ':' in p:
                old, new = p.split(':', 1)
                rename_dict[old.strip()] = new.strip()
        df_to_view.rename(columns=rename_dict, inplace=True)
        current_cols = df_to_view.columns.tolist()

    # 2. Cleaning with exclusions
    if getattr(args, "clean_table", False):
        exclude_spec = getattr(args, "clean_exclude", None)
        exclude_cols = _resolve_columns_advanced(exclude_spec, current_cols) if exclude_spec else []
        df_to_view = _clean_table(
            df_to_view,
            na_rep=getattr(args, "na_rep", "NA"),
            numeric_na=getattr(args, "numeric_na", None),
            exclude=exclude_cols
        )
        current_cols = df_to_view.columns.tolist()

    # 3. Row Sorting and Ranking
    if getattr(args, "sort_by", None):
        sort_spec = args.sort_by.split(',')
        by_cols = []
        ascending_orders = []
        ...
        group_by_spec = getattr(args, "sort_group_by", None)
        if group_by_spec:
            group_cols = _resolve_columns_advanced(group_by_spec, current_cols)
            df_to_view = df_to_view.groupby(group_cols, group_keys=False).apply(
                lambda x: x.sort_values(by=by_cols, ascending=ascending_orders)
            )
        else:
            df_to_view = df_to_view.sort_values(by=by_cols, ascending=ascending_orders)

    elif getattr(args, "top_k", None):
        k = args.top_k
        group_by_spec = getattr(args, "top_k_group_by", None)
        sort_by_spec = getattr(args, "top_k_sort_by", None)
        if not group_by_spec or not sort_by_spec:
            raise ValueError("--top-k requires both --top-k-group-by and --top-k-sort-by.")
        
        group_cols = _resolve_columns_advanced(group_by_spec, current_cols)
        sort_parts = sort_by_spec.strip().split(':')
        sort_col = sort_parts[0]
        sort_order_asc = (sort_parts[1].lower() == 'asc') if len(sort_parts) > 1 else False

        df_to_view = df_to_view.sort_values(by=sort_col, ascending=sort_order_asc).groupby(group_cols).head(k)
    ...
    if getattr(args, "sort_cols_first", None):
        ...
        df_to_view = df_to_view[ordered_cols + other_cols]
    elif getattr(args, "sort_cols_dtype", False):
        string_cols = df_to_view.select_dtypes(include=['object', 'string']).columns.tolist()
        other_cols = [c for c in current_cols if c not in string_cols]
        df_to_view = df_to_view[string_cols + other_cols]
        ...
        df_to_view = df_to_view[special_cols + other_cols]
    
    # 5. Post-processing for integer format and float precision
    ...
    if precision is not None:
        try:
            ...
            pass # Silently ignore invalid precision

    # 6. Output branching
    if getattr(args, "header_view", False):
        _render_vertical_header_view(df_to_view, is_header_present=is_header_present)
        return None

    if getattr(args, "pretty_print", False):
        max_rows = int(getattr(args, "max_rows", 20) or 20)
        _pretty_print_df(
            df_to_view.head(max_rows), is_header_present=is_header_present,
...
def _setup_tbl_parsers(subparsers):
    p = subparsers.add_parser("add-header", help="Add a header to a headerless table.",
                           epilog="Example: tblkit tbl add-header -n id,name,val")
    ...
    p = subparsers.add_parser("clean-header", help="Clean all header names.",
                           epilog="Example: tblkit tbl clean-header")
    ...
    p = subparsers.add_parser("cat", help="Concatenate tables, adding columns from paths.",
                           epilog="Example: tblkit tbl cat --filelist <(find . -name '*.tsv') --ancestor-cols-to-include \"assay,tissue,batch\"")
    ...
    path_extract_group.add_argument("--ancestor-cols-to-include", metavar="COL_NAMES", help="Create columns from parent directories. The first name corresponds to the highest-level ancestor, mapping down to the last name which corresponds to the immediate parent directory.")
    path_extract_group.add_argument("--extract-from-path", metavar="REGEX", help="Advanced: Regex with named capture groups (e.g., 'dir/(?P<id>\\d+)') to create columns from file paths.")
...
def _setup_row_parsers(subparsers):
    ...
    p = subparsers.add_parser("filter", help="Filter rows on a condition.",
                           epilog="Example: tblkit row filter -c msg -p 'ERR'")
    ...
    p = subparsers.add_parser("sample", help="Randomly subsample rows.",
                           epilog="Example: tblkit row sample -n 100 --seed 42")
    ...
    p = subparsers.add_parser("unique", help="Remove duplicate rows.",
                           epilog="Example: tblkit row unique -c user_id")
    ...
    p = subparsers.add_parser("head", help="Return the first N rows.",
                           epilog="Example: tblkit row head -n 5")
    ...
    p = subparsers.add_parser("tail", help="Return the last N rows.",
                           epilog="Example: tblkit row tail -n 5")
    p.add_argument("-n", type=int, default=10, help="Number of rows to return.")
...
def _setup_col_parsers(subparsers):
    p = subparsers.add_parser("rename", help="Rename one or more columns.",
                           epilog="Example: tblkit col rename --map 'c1:id,c2:name'")
    ...
    p = subparsers.add_parser("add", help="Add a new column or add a prefix/suffix.",
                           epilog="Example: tblkit col add -c 1 --new-header status -v active")
    add_columns_arg(p)
    ...
    p = subparsers.add_parser("bin", help="Discretize a numeric column.",
                           epilog="Example: tblkit col bin -c score --bins 5")
    add_columns_arg(p)
    ...
    p = subparsers.add_parser("eval", help="Create column from expression.",
                           epilog="Example: tblkit col eval --expr 'c3 = c1 * c2'")
    ...
    p = subparsers.add_parser("fillna", help="Fill missing values.",
                           epilog="Example: tblkit col fillna -c score --method mean")
    add_columns_arg(p)
    ...
    p = subparsers.add_parser("join", help="Join values from columns.",
                           epilog="Example: tblkit col join -c 'first,last' -o full_name")
    add_columns_arg(p)
    ...
    p = subparsers.add_parser("move", help="Move a column.",
                           epilog="Example: tblkit col move -c id -j name --position before")
    ...
    p = subparsers.add_parser("replace", help="Replace values in a column.",
                           epilog="Example: tblkit col replace -c country --from USA --to 'United States'")
    add_columns_arg(p)
    ...
    p = subparsers.add_parser("scale", help="Scale/normalize numeric columns.",
                           epilog="Example: tblkit col scale -c value --method zscore")
    add_columns_arg(p)
    ...
    p = subparsers.add_parser("select", help="Select columns by name or type.",
                           epilog="Example: tblkit col select -c 'id,name,date'")
    add_columns_arg(p, required=False)
    ...
    p = subparsers.add_parser("split", help="Split a column.",
                           epilog="Example: tblkit col split -c full_name -d ' '")
    add_columns_arg(p)
    ...
    p = subparsers.add_parser("strip", help="Remove pattern from values.",
                           epilog="Example: tblkit col strip -c price -p '[$,]'")
    add_columns_arg(p)
    add_pattern_args(p)
    add_output_arg(p, default_name=None)
    ...
    p = subparsers.add_parser("prune", help="Report or drop uninformative columns.",
                           epilog="Example: tblkit col prune --method zero_variance --execute")
    ...
    p = subparsers.add_parser("cast", help="Change the data type of a column.",
                           epilog="Example: tblkit col cast -c date --to datetime")
    add_columns_arg(p)
...
# PASTE THIS CODE TO REPLACE THE EXISTING _setup_view_parser FUNCTION
def _setup_view_parser(p_view):
    """
    Configure the 'view' command with enhanced sorting, renaming, and cleaning options.
    """
    # General view options
    p_view.add_argument("--precision", type=int, help="Digits to display for floats.")
    p_view.add_argument("--max-rows", type=int, default=20, help="Rows to preview for pretty-print (default: 20).")
    ...
    p_view.add_argument("-H", "--header-view", action="store_true", help="Fast view of header names and first data row.")

    # Header and column renaming
    p_view.add_argument("--rename-map", metavar="'old:new,...'", help="Rename column headers on-the-fly, e.g., 'c1:ID,c2:Value'.")

    # Column sorting options (mutually exclusive)
    col_sort_group = p_view.add_mutually_exclusive_group()
    col_sort_group.add_argument("--sort-cols-first", metavar="COLS", help="Move specified columns to the far left, in order.")
    col_sort_group.add_argument("--sort-cols-dtype", action="store_true", help="Sort columns by type (strings first).")
    col_sort_group.add_argument("--sort-cols-alpha", action="store_true", help="Sort columns alphabetically.")
    p_view.add_argument("--columns-special", metavar="COLS", help="Exempt these columns from --sort-cols-alpha, keeping them at the left.")

    # Row sorting and ranking options
    row_order_group = p_view.add_argument_group("Row Ordering (Sorting and Ranking)")
    row_order_group.add_argument("--sort-by", metavar="'c1:asc,c2:desc'", help="Sort rows by one or more columns.")
    row_order_group.add_argument("--sort-group-by", metavar="COLS", help="Perform sorting within groups defined by these columns.")
    row_order_group.add_argument("--top-k", type=int, metavar="N", help="Select the top N rows per group.")
    row_order_group.add_argument("--top-k-group-by", metavar="COLS", help="Column(s) to group by for top-k selection (required by --top-k).")
    row_order_group.add_argument("--top-k-sort-by", metavar="'col:desc'", help="Column to sort by to determine top rows (required by --top-k).")

    # Cleaning options
    p_view.add_argument("--clean", action="store_true", dest="clean_table",
                        help="Sanitize headers and string values to be machine-readable (snake_case).")
    p_view.add_argument("--clean-exclude", metavar="COLS",
                        help="Comma-separated columns to exclude from header/value cleaning.")
    ...
    p_view.add_argument("--numeric-na", choices=['nan', '0'], help="Representation for NA in numeric columns when cleaning.")

    if "_handle_view" in globals():
        p_view.set_defaults(handler=_handle_view)

    # Subparsers for extra view actions (like 'frequency')
    ...
    pvf.add_argument(
        "-n", "--top-n", dest="top_n", type=int, default=20,
...
def _setup_arg_parser():
    ...
    p_tbl = subparsers.add_parser("tbl", help="Table-level operations", formatter_class=RequiredOptionalHelpFormatter)
    p_row = subparsers.add_parser("row", help="Row-level operations", formatter_class=RequiredOptionalHelpFormatter)
    p_col = subparsers.add_parser("col", help="Column-level operations", formatter_class=RequiredOptionalHelpFormatter)
    ...
    # --- THIS LINE IS THE FIX for the stat help ---
    stat_actions = p_stat.add_subparsers(dest="action", title="Analysis Actions", required=True)
...
def main():
    ...
    # Fast-path for tbl cat command to avoid premature exit
    is_interactive_stdin = sys.stdin.isatty()
    is_cat_command = getattr(args, 'group', None) == 'tbl' and getattr(args, 'action', None) == 'cat'
    if is_cat_command and not args.files and not getattr(args, 'filelist', None) and is_interactive_stdin:
        sys.stderr.write("Error: 'tbl cat' requires a source of files.\n\n")
        sys.stderr.write("Provide files as arguments, with --filelist, or pipe a list of paths, e.g.:\n")
        sys.stderr.write("  find . -name '*.tsv' | ./tblkit_8_25_4.py tbl cat\n")
        sys.exit(1)

    # Set sane defaults for attributes that may not be present in all arg branches
    for _name, _default in (("row_index", None), ("columns", None), ("column_names", None), ("precision", 3), ("max_rows", 20), ("width", 120), ("fullview", False),):
        if not hasattr(args, _name):
            setattr(args, _name, _default)

    # Handle cases where a group is called without a specific action
    ...
    # Special entry for the interactive shell
    ...
    try:
        ...
        if isinstance(processed_df, pd.DataFrame):
            # FIX: Remove all conflicting keys before passing kwargs to the writer
            writer_kwargs = vars(args).copy()
            writer_kwargs.pop('sep', None)
            writer_kwargs.pop('na_rep', None)
