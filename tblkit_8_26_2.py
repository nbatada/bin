...
def _handle_unsupervised_redundant_cols(df: pd.DataFrame, args: argparse.Namespace, **kwargs) -> pd.DataFrame:
    ...
    for col_name in df.columns:
        # Create a content-based hash for the column, ignoring the header name.
        col_values = df[col_name].values
        try:
            # A stable hash for numpy arrays. Using tobytes() is fast and reliable.
            content_hash = hash(col_values.tobytes())
        except AttributeError:
            # Fallback for complex object types that don't support tobytes()
            content_hash = pd.util.hash_pandas_object(df[col_name], index=False).sum()

        if content_hash in seen_hashes:
            # This column's content is a duplicate of a previously seen column.
            original_col = seen_hashes[content_hash]
            duplicates.append({
                'column_to_drop': col_name,
                'duplicate_of': original_col
            })
        else:
            # First time seeing this content, record it.
            seen_hashes[content_hash] = col_name

    if args.execute:
        cols_to_drop = [d['column_to_drop'] for d in duplicates]
        return df.drop(columns=cols_to_drop)
    else:
        # Default behavior: return a report of what would be dropped.
        if not duplicates:
            return pd.DataFrame({'message': ['No redundant columns found.']})
...
def _setup_unsupervised_parsers(subparsers):
  ...
  cat file1.tsv | tblkit unsupervised find-equivalent-columns --key id_col --in file2.tsv""")
    ...
    p = subparsers.add_parser("redundant-cols",
                           ...
                           epilog="""Examples:
  # Get a report of redundant columns
  cat file.tsv | tblkit unsupervised redundant-cols
...
def _setup_tbl_parsers(subparsers):
    ...
    p = subparsers.add_parser("join", help="Merge with a secondary metadata table.",
                           epilog="Example: tblkit tbl join --meta-file meta.tsv --on id")
    p.add_argument("-m", "--meta-file", dest="right", required=True, help="Path to the secondary (metadata) file.")
