...
def _handle_tbl_add_header(df: pd.DataFrame, args: argparse.Namespace, **kwargs) -> tuple[pd.DataFrame, bool]:
...
def _handle_col_extract(df: pd.DataFrame, args: argparse.Namespace, column_names: List[str], **kwargs) -> pd.DataFrame:
    ...
    def extractor(cell):
        ...
        # If a match is found, return all captured groups. Otherwise, return a tuple of "NA"s.
        if match:
            return match.groups()
        return ("NA",) * num_groups
...
def _write_output(df: pd.DataFrame, sep: str, is_header: bool, na_rep: str):
    """Writes the DataFrame to stdout using a manual, robust method."""
    if df is None:
        return
    
    try:
        if is_header and not df.empty:
            sys.stdout.write(sep.join(map(str, df.columns)) + '\n')
        
        # Use itertuples for efficient row-by-row iteration
        for row in df.itertuples(index=False, name=None):
            # Manually build the output string for each row to ensure consistency
            row_values = []
            for item in row:
                if pd.isna(item):
            ...
            sys.stdout.write(sep.join(row_values) + '\n')
    except BrokenPipeError:
        # This happens when the next command in a pipe (e.g., `head`) closes the stream
        try:
            sys.stdout.close()
        except:
            pass
...
def main():
    ...
    try:
        ...
        # --- MODIFICATION START ---
        result = handler(df, args, **handler_kwargs)
        
        output_header_state = is_header_present
        processed_df = result

        if isinstance(result, tuple) and len(result) == 2 and isinstance(result[1], bool):
            processed_df, output_header_state = result
        
        _write_output(processed_df, input_sep, output_header_state, args.na_rep)
        # --- MODIFICATION END ---
