#!/usr/bin/env python3
"""
Dissect: A powerful command-line tool for tabular data manipulation.
...
@process_columns(multi=True)
def _handle_col_capture_regex(df: pd.DataFrame, args: argparse.Namespace, column_names: List[str], **kwargs) -> pd.DataFrame:
...
def _setup_tbl_parsers(subparsers):
    p_t_agg = subparsers.add_parser("tbl_aggregate", help="Group and aggregate data.", description="Example: cat data.tsv | dissect tbl_aggregate --group col1 -c col2 --agg mean")
...
def _setup_row_parsers(subparsers):
    ...
    p_r_grep = subparsers.add_parser("row_grep", help="Filter rows by pattern.", description="Example: ... | dissect row_grep -c 1 -p 'some_pattern'")
...
def _setup_col_parsers(subparsers):
    ...
    p_c_drop = subparsers.add_parser("col_drop", help="Drop column(s).", description="Example: ... | dissect col_drop -c col1,col3")
...
def _setup_arg_parser():
    parser = CustomArgumentParser(prog="dissect", description="A command-line tool for manipulating tabular data.", add_help=False)
...
def _write_output(df: pd.DataFrame, sep: str, is_header: bool, na_rep: str):
    """Writes the DataFrame to stdout."""
    if df is not None and not df.empty:
        try:
            df.to_csv(sys.stdout, sep=sep, index=False, header=is_header, quoting=csv.QUOTE_MINIMAL, na_rep=na_rep)
