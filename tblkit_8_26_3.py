...
def _setup_col_parsers(subparsers):
  ...
  tblkit col move -c 'user_id,sample_id' --to-positions 1,2""")
    ...
    p = subparsers.add_parser("rename", help="Rename a column header.", epilog="Example: tblkit col rename --map 'old_name:new_name'")
    p.add_argument("--map", required=True, help="A comma or semicolon-separated map of old:new names.")
    ...
    p = subparsers.add_parser("split", help="Split a column into multiple new columns.", epilog="Example: tblkit col split -c full_name -d ' '")
    add_columns_arg(p)
