...
class MainHelpFormatter(EnhancedHelpFormatter):
    ...
    def _format_action(self, action):
        # Let the parent handle all actions except for the subparser one.
        if not isinstance(action, argparse._SubParsersAction):
            return super()._format_action(action)

        # Correctly get the help text for each subcommand from the internal list.
        help_map = {ca.dest: (ca.help or '') for ca in getattr(action, '_choices_actions', [])}
        
        max_len = max((len(name) for name in action.choices), default=0)
...
class CustomArgumentParser(argparse.ArgumentParser):
    ...
    def error(self, message: str):
        ...
        error_line = f"Error: {message.capitalize()}"
...
class CommandGroupHelpFormatter(argparse.RawTextHelpFormatter):
    ...
    def _format_action(self, action):
        ...
        if action.nargs == argparse.PARSER:
            # Get the help text for each subcommand from the internal list.
            help_map = {ca.dest: (ca.help or '') for ca in getattr(action, '_choices_actions', [])}
            ...
            for name in sorted(action.choices.keys()):
                if len(name) > max_len:
                    max_len = len(name)
                help_text = help_map.get(name, action.choices[name].description or '')
            ...
            # Add color and manual padding for correct alignment.
            cyan, reset = ("\033[96m", "\033[0m") if sys.stdout.isatty() and os.getenv("NO_COLOR") is None else ("", "")
            
            lines = [f"\n{title}\n", "-" * len(title) + "\n"]
            # Use manual padding to handle ANSI color codes correctly.
...
def _handle_tbl_join(df, args, column_names=None, **kwargs):
    ...
    if getattr(args, "meta", None) is None:
        raise ValueError("tbl join: --meta path is required")
    ...
    if qchar:
        ...
        read_kw = dict(sep=sep, encoding=enc, escapechar=esc,
                       doublequote=dquote, quoting=quoting, header=header,
                       na_values=na_vals)
    right_df = pd.read_csv(getattr(args, "meta"), **read_kw)

    how = getattr(args, "how", "inner") or "inner"
    on  = getattr(args, "on", None)
    left_on = getattr(args, "left_on", None)
    right_on = getattr(args, "right_on", None)
    ...
    out = df.merge(right_df, how=how, on=on, left_on=left_on, right_on=right_on, suffixes=suffixes)

    if getattr(args, "remove_redundant_cols", False):
        cols_to_drop = set()
        rename_map = {}
        cols_to_check = out.columns.tolist()
        left_sfx, right_sfx = suffixes
        ...
        for i in range(len(cols_to_check)):
            for j in range(i + 1, len(cols_to_check)):
                ...
                if c1 in cols_to_drop or c2 in cols_to_drop:
                    continue
                
                try:
                    is_redundant = ((out[c1] == out[c2]) | out[c1].isna() | out[c2].isna()).all()
                    
                    if is_redundant:
                        cols_to_drop.add(c2)
                        
                        if c1.endswith(left_sfx) and c2.endswith(right_sfx):
                            base_name1 = c1[:-len(left_sfx)]
                            base_name2 = c2[:-len(right_sfx)]
                            if base_name1 == base_name2:
                                rename_map[c1] = base_name1
                except TypeError:
                    continue
        
        if cols_to_drop:
            out = out.drop(columns=list(cols_to_drop))
        if rename_map:
...
def _setup_unsupervised_parsers(subparsers):
  ...
  cat file1.tsv | tblkit unsupervised find-equivalent-columns --key id_col --in file2.tsv""")
    ...
    p = subparsers.add_parser("redundant-cols",
                           help="Find and remove redundant columns based on their content.",
                           epilog="""Examples:
  # Get a report of redundant columns, ignoring NAs
  cat file.tsv | tblkit unsupervised redundant-cols --partial

  # Remove the redundant columns from the table
  cat file.tsv | tblkit unsupervised redundant-cols --execute""")
    p.add_argument("--execute", action="store_true", help="Remove the redundant columns instead of printing a report.")
    p.add_argument("--partial", action="store_true", help="Allow partial overlap (ignores NA values during comparison).")
...
def _setup_tbl_parsers(subparsers):
    ...
    p = subparsers.add_parser("join", help="Merge with a secondary metadata table.",
                           epilog="Example: tblkit tbl join --meta meta.tsv --on id -e")
    p.add_argument("-m", "--meta", required=True, help="Path to the secondary (metadata) file.")
...
def _handle_unsupervised_impute(df: pd.DataFrame, args: argparse.Namespace, **kwargs) -> pd.DataFrame:
    """
    Imputes missing values using simple unsupervised strategies (median, mean, or mode).
    Can operate globally or on a per-group basis.
    """
    df_out = df.copy()
    report = []
    ...
    def impute_group(sub_df):
        # Numeric imputation
        numeric_cols = sub_df.select_dtypes(include='number').columns
        for col in numeric_cols:
            if sub_df[col].isna().any():
                if args.numeric == 'median':
                    impute_val = sub_df[col].median()
                else: # mean
                    impute_val = sub_df[col].mean()
                sub_df[col].fillna(impute_val, inplace=True)
                report.append({'column': col, 'strategy': args.numeric, 'value': impute_val, 'group': sub_df.name if group_cols else 'global'})

        # Categorical imputation
        categorical_cols = sub_df.select_dtypes(include=['object', 'category']).columns
        for col in categorical_cols:
            if sub_df[col].isna().any():
                impute_val = sub_df[col].mode()[0] if not sub_df[col].mode().empty else pd.NA
                sub_df[col].fillna(impute_val, inplace=True)
                report.append({'column': col, 'strategy': 'mode', 'value': impute_val, 'group': sub_df.name if group_cols else 'global'})
        return sub_df

    if group_cols:
        df_out = df_out.groupby(group_cols, group_keys=False).apply(impute_group)
    else:
        df_out = impute_group(df_out)
            
    if args.execute:
        return df_out
    else:
        return pd.DataFrame(report, columns=['column', 'strategy', 'value', 'group']) if report else pd.DataFrame()
...
def main():
    ...
    # If no command is given, or just a group name, show help.
    if len(sys.argv) == 1:
        parser.print_help()
        sys.exit(0)
    
    if len(sys.argv) == 2 and sys.argv[1] in parser._subparsers._group_actions[0].choices:
        # This triggers the clean, high-level help for a command group
    ...
    if df.empty and not is_additive_op:
        if len(df.columns) > 0:
            ...
            # Remove keys that are passed positionally to avoid TypeError
            writer_kwargs.pop('sep', None)
            writer_kwargs.pop('output_sep', None)
