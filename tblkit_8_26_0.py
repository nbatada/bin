...
def _handle_tbl_sanitize(df: pd.DataFrame, args: argparse.Namespace, **kwargs) -> pd.DataFrame:
    """Sanitizes all headers and string values to be machine-readable."""
    is_header_present = kwargs.get('is_header_present', True)
    if not is_header_present:
        return df # Nothing to clean if no header
    
    exclude_spec = getattr(args, "exclude", None)
    exclude_cols = _resolve_columns_advanced(exclude_spec, df.columns) if exclude_spec else []
...
def _setup_arg_parser():
    ...
    # ... (rest of the I/O options are unchanged, keep them as they are) ...
    ...
    subparsers = parser.add_subparsers(
        ...
        metavar="{tbl,subset,row,col,stat,unsupervised,view,explore}",
    )
...
def _setup_tbl_parsers(subparsers):
    ...
    # ... (rest of join arguments are unchanged) ...
    ...
    p = subparsers.add_parser("sanitize", help="Clean all headers and values for downstream processing.",
                           epilog="Example: tblkit tbl sanitize -E id_col")
    p.add_argument("-E", "--exclude", metavar="COLS", help="Comma-separated columns to exclude from cleaning.")
    p.add_argument("--numeric-na", choices=['nan', '0'], help="Representation for NA in numeric columns.")
    p.add_argument("--coerce-numeric", action="store_true", help="Convert numeric-looking strings to numbers.")
    p.add_argument("--parse-datetime", action="store_true", help="Try parsing common date formats.")
    p.set_defaults(handler=_handle_tbl_sanitize)


def _setup_col_parsers(subparsers):
    # This function is now shorter as feature-dedup has been removed.
    p = subparsers.add_parser("rename", help="Rename one or more columns.", epilog="Example: tblkit col rename --map 'c1:id,c2:name'")
    # ... (all other col commands like add, bin, cast, etc., remain here) ...
    ...
    p = subparsers.add_parser("clean-values", help="Clean string values.", epilog="Example: tblkit col clean-values -c category")
    add_columns_arg(p)
    ...
    p = subparsers.add_parser("extract", help="Extract regex capture group(s).", epilog="Example: tblkit col extract -c url -p 'id=(\\d+)'")
    add_columns_arg(p)
    add_pattern_args(p)
    ...
    p = subparsers.add_parser("move", help="Move a column.", epilog="Example: tblkit col move -c id -j name --position before")
    ...
    p = subparsers.add_parser("replace", help="Replace values in a column.", epilog="Example: tblkit col replace -c country --from USA --to 'United States'")
    add_columns_arg(p)
    ...
    p = subparsers.add_parser("strip", help="Remove pattern from values.", epilog="Example: tblkit col strip -c price -p '[$,]'")
    add_columns_arg(p)
    add_pattern_args(p)
    add_output_arg(p, default_name=None)
...
def _setup_subset_parsers(subparsers):
    ...
    p = subparsers.add_parser("unique", help="Remove duplicate rows.",
                           epilog="Example: tblkit subset unique -c user_id")
    add_columns_arg(p, required=False)
...
def _setup_view_parser(p_view):
    ...
    p_view.add_argument("--clean", action="store_true", dest="clean_table",
                        help="Sanitize headers and string values for final presentation.")
    p_view.add_argument("-E", "--clean-exclude", metavar="COLS",
                        help="Comma-separated columns to exclude from --clean.")
