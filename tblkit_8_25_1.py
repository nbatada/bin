...
def _handle_view(df: pd.DataFrame, args: argparse.Namespace, is_header_present: bool, **kwargs) -> Optional[pd.DataFrame]:
    ...
    # Re-order: show a row id column first if requested
    row_index = getattr(args, "row_index", None)
    if row_index and not df_to_view.empty:
        row_idx_col_name = _parse_single_col(row_index, df_to_view.columns, is_header_present)
    ...
    if getattr(args, "clean_table", False):
        exclude = []
        clean_exclude = getattr(args, "clean_exclude", None)
        if clean_exclude:
            exclude = _parse_multi_cols(clean_exclude, df_to_view.columns, is_header_present)
    ...
    # Precision (stringify floats with significant digits)
    precision = getattr(args, "precision", None)
    if precision is not None:
        try:
            df_to_view = _stringify_floats_sig(df_to_view, int(precision))
        except Exception:
            # Fall back silently if precision is invalid
            pass
    ...
    if getattr(args, "header_view", False):
        _pretty_print_df(
            ...
            precision=None  # already stringified if precision was set
        )
        sys.exit(0)
    ...
    # Default: raw/cleaned echo
    _write_output(
        df_to_view,
        sep=output_sep,
        is_header=is_header_present,
        na_rep=getattr(args, "na_rep", "NA"),
        quotechar=getattr(args, "quotechar", None),
        escapechar=getattr(args, "escapechar", None),
        doublequote=bool(getattr(args, "doublequote", True)),
    )
    return None
...
# --- paste start: _setup_view_parser ---
def _setup_view_parser(p_view):
    """
    Configure the 'view' command. Keeps the direct 'view' handler and
    also adds 'view frequency' as a subcommand.
    """

    # Arguments for the direct 'view' command (safe defaults)
    p_view.add_argument("--precision", type=int, default=3,
                        help="Digits to display for floats (default: 3).")
    p_view.add_argument("--max-rows", type=int, default=20,
                        help="Rows to preview (default: 20).")
    p_view.add_argument("--width", type=int, default=120,
                        help="Display width (default: 120).")
    p_view.add_argument("--fullview", action="store_true",
                        help="Show all rows (caution: large output).")

    # If your code defines _handle_view for the direct 'view' command, bind it.
    if "_handle_view" in globals():
        p_view.set_defaults(handler=_handle_view)

    # Subparsers for extra view actions
...
def main():
    ...
    # --- paste start (defensive default) ---
    # Some handlers assume these attributes exist; ensure they’re present.
